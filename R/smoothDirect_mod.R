# Modified version of smoothDirect() to incorporate woman age adjustment

smoothDirect_mod <- function (
  data, Amat, X = NULL, formula = NULL, time.model = c("rw1", "rw2", "ar1")[2],
  st.time.model = NULL, year_label, year_range = c(1980,2014), is.yearly = TRUE,
  m = 5, type.st = 1, survey.effect = FALSE, hyper = c("pc", "gamma")[1],
  pc.u = 1, pc.alpha = 0.01, pc.u.phi = 0.5, pc.alpha.phi = 2/3, pc.u.cor = 0.7,
  ...
  if (isTRUE(requireNamespace("INLA", quietly = TRUE))) {
    ...
    if (is.yearly) {
      ...
      years <- data.frame(year = year_label_new[1:N], 
                          year_number = seq(1, N))
    }
    else {
      n <- 0
    ...
    if (!is.null(X)) {
      ...
      covariate.names <- colnames(X)[colnames(X) %in% 
                                       by == FALSE]
      ...
      if (tolower(hyper) == "pc") {
        ...
        if (!is.yearly && !is.spatial) {
          ...
          formula <- logit.est ~ f(time.struct, model = rw.model.pc, 
                                   ...
                                   values = 1:N) + f(time.unstruct, model = iid.model.pc)
        }
        else if (!is.yearly && (is.spatial)) {
          formula <- logit.est ~ f(time.struct, model = paste0("rw", 
                                                               rw), constr = TRUE, extraconstr = period.constr, 
          ...
          formula <- logit.est ~ f(time.struct, model = rw.model.pc, 
            ...
            f(region.struct, graph = Amat, model = "bym2", 
              ...
              adjust.for.con.comp = TRUE) + f(time.area, 
                                              model = st.model.pc, diagonal = 1e-06, extraconstr = constr.st, 
                                              values = 1:(N * S))
        }
        if (survey.effect) {
        ...
        if (!is.yearly && !is.spatial) {
          ...
          formula <- logit.est ~ f(time.struct, model = rw.model, 
                                   diagonal = 1e-06, extraconstr = constr, 
                                   values = 1:N) + f(time.unstruct, model = iid.model)
        }
        else if (!is.yearly && (is.spatial)) {
          formula <- logit.est ~ f(time.struct, model = paste0("rw", 
          ...
          formula <- logit.est ~ f(time.struct, model = rw.model, 
            ...
            f(region.struct, graph = Amat, model = "besag", 
              ...
              adjust.for.con.comp = TRUE) + f(region.unstruct, 
                                              model = "iid", param = c(a.iid, b.iid)) + 
            f(time.area, model = st.model, diagonal = 1e-06, 
    ...
    if (is.spatial) {
      ...
      for (j in 1:region_count) {
        for (i in 1:N) {
          ...
          if (!is.null(X)) {
            if ("region" %in% by && (!"years" %in% by)) {
              which <- which(exdat$region.struct == 
                               j)
            }
            else if ((!"region" %in% by) && "years" %in% 
                     by) {
          ...
          lincombs.info[index, c("District", "Year")] <- c(j, 
                                                           i)
          if (is.ar && type.st == 1) {
            tmplin <- list(`(Intercept)` = 1, time.area = spacetime, 
                           time.struct = time, time.unstruct = time, 
                           region.struct = area)
            if (is.main.ar) 
              tmplin <- c(tmplin, time.slope = (i - 
                                                  center)/(N - 1))
            ...
            assign(object.name, INLA::inla.make.lincomb(c(list(`(Intercept)` = 1, 
                                                               time.area = spacetime, time.struct = time, 
                                                               time.unstruct = time, region.struct = area), 
                                                          XX)))
          }
          else {
            ...
            assign(object.name, INLA::inla.make.lincomb(c(list(`(Intercept)` = 1, 
                                                               time.struct = time, time.unstruct = time, 
                                                               region.struct = area, region.int = newidx), 
                                                          XX)))
          }
          if (index == 1) {
